...
// Import da Health: calcola i cuori in due modi e prende il più alto, per premiare
// chi usa Health Connect/Apple Health con dati più ricchi (km, dislivello, tempo)
// 1) da calorie Health (fonte primaria per il confronto)
// 2) da km (corsa) / km+dislivello (camminata) / durata (altre attività)
function calcRunCoolResult(workout: ImportedWorkout): { hearts: number; calories: number; usedKm: boolean; fromHealth: boolean } {
  ...
  const candidates: { hearts: number; calories: number; usedKm: boolean; fromHealth: boolean }[] = [];

  // Candidato da calorie Health
  if (workout.caloriesBurned > 0) {
    const calories = Math.round(workout.caloriesBurned);
    candidates.push({ hearts: calcHeartsGained(calories, wId), calories, usedKm: false, fromHealth: true });
  }

  // Candidato da km (corsa) o km+dislivello (camminata)
  if (def.inputType === 'km' && workout.distanceKm && workout.distanceKm > 0) {
    const calories = Math.round((def.calPerKm ?? 60) * workout.distanceKm);
    candidates.push({ hearts: calcHeartsGained(calories, wId), calories, usedKm: true, fromHealth: false });
  } else if (def.inputType === 'km_elevation' && workout.distanceKm && workout.distanceKm > 0) {
    const calories = calcWalkingCalories(workout.distanceKm, workout.elevationMeters ?? 0);
    const hearts = Math.floor((workout.distanceKm + (workout.elevationMeters ?? 0) * 0.03) / 6);
    candidates.push({ hearts, calories, usedKm: true, fromHealth: false });
  }

  // Candidato da durata (attività a tempo)
  if (def.inputType === 'duration' && workout.durationMinutes > 0) {
    const calories = Math.round((def.calPerMin ?? 7) * workout.durationMinutes);
    const hearts = calcHeartsFromDuration(workout.durationMinutes, def.heartsPerHour ?? 3);
    candidates.push({ hearts, calories, usedKm: false, fromHealth: false });
  }

  if (candidates.length === 0) return { hearts: 0, calories: 0, usedKm: false, fromHealth: false };

  // Prende il candidato con più cuori (a parità, preferisce quello non-Health)
...
function makeStyles(colors: ThemeColors, isDark: boolean) {
  return StyleSheet.create({
    ...
    connectingText: { marginTop: 16, fontSize: 14, color: colors.textDim },
    unavailableTitle: { fontSize: 20, fontWeight: '800', color: colors.text, marginBottom: 8 },
    unavailableSub: { fontSize: 14, color: colors.textDim, textAlign: 'center', lineHeight: 20 },
    ...
    headerCard: {
      ...
      shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: isDark ? 0 : 0.06, shadowRadius: 8, elevation: isDark ? 0 : 3,
    },
    headerEmoji: { fontSize: 36, marginBottom: 8 },
    headerTitle: { fontSize: 18, fontWeight: '800', color: colors.text, marginBottom: 4 },
    headerSub: { fontSize: 13, color: colors.textDim },

    emptyCard: {
      backgroundColor: colors.card, borderRadius: 16, padding: 32, alignItems: 'center',
      marginBottom: 16,
    },
    emptyEmoji: { fontSize: 40, marginBottom: 12 },
    emptyText: { fontSize: 15, fontWeight: '600', color: colors.textDim, textAlign: 'center', marginBottom: 6 },
    emptySub: { fontSize: 12, color: colors.textFaint, textAlign: 'center', lineHeight: 18 },

    workoutCard: {
      backgroundColor: colors.card, borderRadius: 16, padding: 16, marginBottom: 10,
      shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: isDark ? 0 : 0.05, shadowRadius: 4, elevation: isDark ? 0 : 2,
    },
    workoutCardImported: { borderWidth: 2, borderColor: '#4CAF50', backgroundColor: isDark ? '#16291a' : '#f0fff0' },
    ...
    workoutDate: { fontSize: 12, color: colors.textFaint, fontWeight: '600' },
    ...
    importedBadge: { backgroundColor: isDark ? '#1c3320' : '#e8f5e9', borderRadius: 10, padding: 12, alignItems: 'center' },
    importedText: { fontSize: 14, fontWeight: '700', color: isDark ? '#7ed896' : '#2e7d32' },

    backBtn: { alignSelf: 'center', marginTop: 8, paddingVertical: 12, paddingHorizontal: 24 },
    backBtnText: { fontSize: 14, fontWeight: '600', color: colors.textDim },
  });
}
